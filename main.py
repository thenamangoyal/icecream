...
class IceCreamGame():
    def __init__(self, args):
        ...
        for i in range(1, self.total_turn_per_player + 1):
            ...
            if i != 1:
                self.game['cum_turn_' + str(i)] = [{}] * len(self.game)
            for j in range(len(self.players)):
                self.game.at[j, 'turn_' + str(i)] = {k: 0 for k in self.flavors}
                if i != 1:
                    self.game.at[j, 'cum_turn_' + str(i)] = {k: 0 for k in self.flavors}

        self.next_player = self.__assign_next_player()

        self.game.at[self.next_player, 'turn'] += 1
    ...
    def __add_player(self, player_class, player_name):
        if player_name not in self.player_names:
            ...
            row = pd.DataFrame({'preference': np.nan, 'turn': 0, 'total': np.nan,
                                'score': 0.0, 'avg_score': 0.0}, index=[player_idx])
            row['preference'] = row['preference'].astype(object)
            row['total'] = row['total'].astype(object)
            self.game = self.game.append(row, ignore_index=False)
            self.game.at[player_idx, 'preference'] = player_preference
            self.game.at[player_idx, 'total'] = {k: 0 for k in self.flavors}
    ...
    def __game_end(self):
        if not self.end_message_printed and self.is_game_ended():
            ...
            for player_idx, score in enumerate(self.player_scores):
                ...
                self.game.at[player_idx, 'score'] = score
            ...
            for player_idx, score in enumerate(self.player_scores):
                ...
                self.game.at[player_idx, 'avg_score'] = final_scores[player_idx]
            ...
            self.game.to_csv('game_records/' + str(self.seed) + '_game.csv')
            data = pd.DataFrame(columns=['seed', 'player', 'num_players', 'num_flavors', 'current_turn', 'flavor',
                                         'quantity_taken', 'reward'])
            for p in range(len(self.players)):
                for t in range(1, self.total_turn_per_player + 1):
                    for f in self.flavors:
                        if t == 1:
                            q = self.game.at[p, 'turn_' + str(t)][f]
                        else:
                            q = self.game.at[p, 'cum_turn_' + str(t)][f]
                        row = pd.DataFrame({'num_players': len(self.players), 'num_flavors': len(self.flavors),
                                            'current_turn': t, 'flavor': f, 'quantity_taken': q, 'player': p,
                                            'reward': (len(self.flavors) - self.__get_flavor_preference(p, f) + 1),
                                            'seed': self.seed}, index=[0])
                        data = data.append(row, ignore_index=True)
            if os.path.exists("data_f_"+ str(len(self.flavors))+ "_p_" +str(len(self.players))+ ".csv"):
                p_data = pd.read_csv("data_f_"+ str(len(self.flavors))+ "_p_" +str(len(self.players))+ ".csv")
                data = data.append(p_data, ignore_index=True)
            data.to_csv("data_f_"+ str(len(self.flavors))+ "_p_" +str(len(self.players))+ ".csv", index=False)
            ...
            if is_valid_action:
                ...
                if action == "scoop":
                    ...
                    if not(i >= 0 and i < self.l-1 and j >= 0 and j < self.w-1):
                        ...
                        t = self.game.at[player_idx, 'turn']
                        d = self.game.at[player_idx, 'turn_' + str(t)]
                        dd = self.game.at[player_idx, 'total']

                        for s in scooped_items:
                            d[s] = d[s] + 1
                            dd[s] = dd[s] + 1

                        if t != 1:
                            self.game.at[player_idx, 'cum_turn_' + str(t)] = copy.copy(dd)
