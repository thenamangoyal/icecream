...
class Player:
    ...
    def serve(self, top_layer: np.ndarray, curr_level: np.ndarray, player_idx: int, get_flavors: Callable[[], List[int]], get_player_count: Callable[[], int], get_served: Callable[[], List[Dict[int, int]]], get_turns_received: Callable[[], List[int]]) -> Dict[str, Union[Tuple[int], int]]:
        ...
        if self.state < 24:
            ...
            self.nextPlayerPreference = Player.estimateNextScore(player_to_pass ,  top_layer , curr_level ) 
            values = player_to_pass
            self.previousNextPlayer = player_to_pass
        return {"action": action,  "values": values}


    def estimateNextScore(player_to_pass: int ,top_layer: np.ndarray, curr_level: np.ndarray, get_served: Callable[[], List[Dict[int, int]]]):
        ...
        for permutation in permutations:
            ...
            score, permutation_dict[preferences] =  self.calculateBestScoopForEachPlayer(top_layer, curr_level, player_preferences)
    ...
    def calculateBestScoop(self, top_layer: np.ndarray, curr_level: np.ndarray, get_flavors: Callable[[], List[int]], player_list, get_served: Callable[[], List[Dict[int, int]]], get_turns_received: Callable[[], List[int]]) -> Dict[str, Union[Tuple[int], int]]:
        ...
        for player in player_list:
            ...
            temp.reverse()
            player_preferences = temp
            players_score, number_of_cubes_per_flavor = self.calculateBestScoopForEachPlayer(top_layer, curr_level, player_preferences)
            if players_score > max_players_score:
                player_to_pass = player
                max_players_score = players_score
        return player_to_pass       

    def calculateBestScoopForEachPlayer(self, top_layer: np.ndarray, curr_level: np.ndarray, player_preferences: List[int]):
        number_of_cubes_per_flavor = {}
        players_score = 0
        counter = 1
        cells_taken = []
        while len(cells_taken) < 24 and counter != len(cells_taken):
            ...
            if (i, j) not in cells_taken and curr_level[i][j] == CL:
                ...
                number_of_cubes_per_flavor[player_preferences.index(top_layer[i][j])] += 1
            if (i+1, j) not in cells_taken and curr_level[i+1][j] == CL:
                cells_taken.append((i+1, j))
                number_of_cubes_per_flavor[player_preferences.index(top_layer[i+1][j])] += 1
            if (i, j+1) not in cells_taken and curr_level[i][j+1] == CL:
                cells_taken.append((i, j+1))
                number_of_cubes_per_flavor[player_preferences.index(top_layer[i][+1j])] += 1
